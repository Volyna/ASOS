...
const ProductsPage = () => {
   ...
   const pro =  useSelector((state:RootState) => state.allProducts.products)
    ...
    const {id, name, price, discount, description, color, size, brand, quantity, inStock, isFavourite, image, categ} = products;
    console.log(image);
    ...
    return(
        ...
        <div className="itemName">
                    <div className="itemImg">
                        <img className="titleImg" src={'data:image/jpeg;base64,' + `${image}`}/>
                        <img className='like' src={like}/>
                    </div>

                    <div className="info">
                        <p className='headerText' id="name">{name}</p>
                        <p className='headerText' id="price">{price}$</p>
